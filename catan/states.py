...
class GameStateNotInGameMoveRobber(GameStateNotInGame):
    ...
    def move_robber(self, tile_id):
        ...
        if len(robbers) != 1:
            logging.warning(
                '{} robbers found in board.pieces'.format(len(robbers)))
...
class GameStatePreGame(GameStateInGame):
    ...
    def next_player(self):
        ...
        try:
            ...
            self.game.set_state(GameStateBeginTurn(self.game))
            self.game.board.state.pregame = False # Set pregame to false
...
class GameStatePreGamePlacingPiece(GameStatePreGame):
    ...
    """
    # ADDED PREV SETTLEMENT TO ENSURE ROAD CAN ONLY BE PLACED ON LAST PLACED SETTLEMENT

    def __init__(self, game, piece_type, prev_settlement=None):
        super(GameStatePreGamePlacingPiece, self).__init__(game)
        self.piece_type = piece_type
        self.prev_settlement = prev_settlement
...
class GameStatePlacingRoadBuilderPieces(GameStatePlacingPiece):
    ...
    def __init__(self, game):
        super(GameStatePlacingRoadBuilderPieces, self).__init__(
            game, catan.pieces.PieceType.road)
        self.edges = list()
    ...
    def place_road(self, edge):
        ...
        piece = catan.pieces.Piece(
            catan.pieces.PieceType.road, self.game.get_cur_player())
...
class BoardState(object):
    def __init__(self, board, pregame=False):
        self.board = board
