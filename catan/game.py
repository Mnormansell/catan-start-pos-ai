...
class Game(object):
    ...
    def get_cur_player(self):
        if self._cur_player is None:
            ...
            # p = self._cur_player
            # return Player(p.seat, p.name, p.color, agent=p.agent)

    def set_cur_player(self, player):
        self._cur_player = Player(player.seat, player.name, player.color, agent=player.agent)
    ...
    def begin_placing(self, piece_type):
        if self.state.is_in_pregame():
            ...
            # Call agent (To solve road placement) 
            # # TODO: Expand AI Past pregame?
            if self.get_cur_player().agent:
                self.get_cur_player().agent.solve(self.state)
        else:
    ...
    def buy_settlement(self, node):
        ...
        if self.state.is_in_pregame():
            self.set_state(catan.states.GameStatePreGamePlacingPiece(self, catan.pieces.PieceType.road, prev_settlement=node))
            # Call agent (To solve road placement) 
            # # TODO: Expand AI Past pregame?
            if self.get_cur_player().agent:
                self.get_cur_player().agent.solve(self.state)
...
class Player(object):
    ...
    :param agent: will be Agent in catan.agent, default none (represents if manual player or computer agent)
    """
    def __init__(self, seat, name, color, agent=None):
        if not (1 <= seat <= 4):
    ...
    def __repr__(self):
        return '{} ({}) ({})'.format(self.color, self.name, self.agent)
