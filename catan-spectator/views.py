...
from utils.tile_images import gen_tile_images
from PIL import ImageTk

logging.basicConfig(level=logging.CRITICAL)

# Translate terrain type -> background imgae
IMAGE_DICT = gen_tile_images()
# Store references to ImageTK so it isn't deleted
IMAGEZ = {}
...
class BoardFrame(tkinter.Frame):
    ...
    def _draw_tile(self, x, y, terrain, tile):
        self._draw_hexagon(self._tile_radius, offset=(x, y), terrain=terrain, tags=self._tile_tag(tile))

    # def _draw_tile(self, x, y, terrain, tile):
    #     self._draw_hexagon(self._tile_radius, offset=(x, y), fill=self._colors[terrain], tags=self._tile_tag(tile))

    def _draw_hexagon(self, radius, offset=(0, 0), rotate=30, terrain=Terrain.desert, tags=None):
        points = self._hex_points(radius, offset, rotate)
        image = ImageTk.PhotoImage(IMAGE_DICT[terrain.value])
        IMAGEZ[offset] = image # Store image so it doesn't get deleted
        self._board_canvas.create_image(*offset,image=image)
    ...
    def _draw_piece_shadows(self, piece_type, board, terrain_centers):
        ...
        if piece_type == PieceType.road:
            ...
            # If we are in pregame, we can only place roads at settlement we just placed
            if self.game.state.is_in_pregame():
                prev_settlement = self.game.state.prev_settlement
                for edge in edges: 
                    if prev_settlement in hexgrid.nodes_touching_edge(edge) and self._board.can_place_piece(piece, edge):
                        self._draw_piece(edge, piece, terrain_centers, ghost=True)
            # If we aren't in pregeme we can place settlements anywhere open spot touching our settlement
            else: 
                for edge in edges:
                    if self._board.can_place_piece(piece, edge):
                        self._draw_piece(edge, piece, terrain_centers, ghost=True)
            ...
            for node in nodes:
                if self._board.can_place_piece(piece, node):
                    self._draw_piece(node, piece, terrain_centers, ghost=True)
    ...
    def _piece_tkinter_opts(self, coord, piece, **kwargs):
        ...
        opts['outline'] = 'black' #Changed from Color
        opts['fill'] = color
...
class SetupGameToolbarFrame(tkinter.Frame):
    ...
    def __init__(self, master, game, options=None, *args, **kwargs):
        ...
        self.start_game_player_order = StartGamePlayerOrderFrame(self, game, **self.options)
    ...
    def on_start_game(self):
        ...
        def get_agent(avar, heuristic, player):
            ...
            if agent == 'Tree Agent':
                return TreeAgent(self.game, player, heuristic=heuristic)
