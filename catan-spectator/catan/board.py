...
class Board(object):
    ...
    def __init__(self, board=None, terrain=None, numbers=None, ports=None, pieces=None, players=None):
        ...
        # Set graph to pass in "completed" board
        self.graph = CatanGraph(self)
    ...
    def lock(self):
        self.state = states.BoardStateLocked(self, pregame=self.state.pregame)
    ...
    def unlock(self):
        self.state = states.BoardStateModifiable(self, pregame=self.state.pregame)
    ...
    def can_place_piece(self, piece, coord):
        if piece.type == PieceType.road:
            # Cannot place if currently occupied
            if (0, coord) in self.pieces.keys():
                return False
            
            # Check if road placement borders an existing settlemnt
            bordering_nodes = hexgrid.nodes_touching_edge(coord)
            for node in bordering_nodes:
                if (1, node) in self.pieces.keys() and self.pieces[(1, node)].owner == piece.owner:
                    return True
            
            # In pregame we have to come from existing settlement
            if self.state.pregame: return False

            # Check if we can come from an existing road
            for node in bordering_nodes:
                # If this is true then it is occupied by another player (since check about failed), so we continue
                if (1, node) in self.pieces.keys():
                    continue

                # Check if we can continue an existing road
                for edge in self.graph.get_edges_of_node(node):
                    # This will check current edge but it will fail as its not in the keys
                    if (0, edge) in self.pieces.keys() and self.pieces[(0, edge)].owner == piece.owner:
                        return True

            return False
        elif piece.type == PieceType.settlement:
            # Cannot place if currently occupied
            if (1, coord) in self.pieces.keys():
                return False
            ...
            # If in pregame it's valid as we don't need incoming road
            if self.state.pregame: return True

            # Check if we own an inbound road
            for edge in self.graph.get_edges_of_node(coord):
                if (0, edge) in self.pieces.keys() and self.pieces[(0, edge)].owner == piece.owner:
                    return True
    ...
    def place_piece(self, piece, coord):
        if not self.can_place_piece(piece, coord):
            raise ValueError('ILLEGAL: Attempted to place piece={} on coord={}'.format(
                piece.type, hex(coord)
            ))
    ...
    def move_piece(self, piece, from_coord, to_coord):
        ...
        if from_index not in self.pieces:
            logging.warning(
                'Attempted to move piece={} which was NOT on the board'.format(from_index))
            return
    ...
    def remove_piece(self, piece, coord):
        ...
        try:
            ...
            logging.critical(
                'Attempted to remove piece={} which was NOT on the board'.format(index))

    def get_pieces(self, types=tuple(), coord=None):
        if coord is None:
            logging.critical(
                'Attempted to get_piece with coord={}'.format(coord))
        ...
        if len(pieces) == 0:
            ...
            logging.debug(
                'Found one piece at {}: {}'.format(indexes, pieces[0]))
        elif len(pieces) > 1:
            logging.debug('Found {} pieces at {}: {}'.format(
                len(pieces), indexes, coord, pieces))
        return pieces
    ...
    def _piece_type_to_hex_type(self, piece_type):
        if piece_type in (PieceType.road, ):
            ...
            logging.critical(
                'piece type={} has no corresponding hex type. Returning None'.format(piece_type))
            return None
    ...
    def cycle_hex_type(self, tile_id):
        if self.state.modifiable():
            ...
            logging.debug(
                'Attempted to cycle terrain on tile={} on a locked board'.format(tile_id))
        self.notify_observers()

    def cycle_hex_number(self, tile_id):
        if self.state.modifiable():
            tile = self.tiles[tile_id - 1]
            next_idx = (list(HexNumber).index(
                tile.number) + 1) % len(HexNumber)
            next_hex_number = list(HexNumber)[next_idx]
            tile.number = next_hex_number
        else:
            logging.debug(
                'Attempted to cycle number on tile={} on a locked board'.format(tile_id))
        self.notify_observers()
    ...
    def cycle_port_type(self, tile_id, direction):
        if self.state.modifiable():
            ...
            logging.debug('Attempted to cycle port on coord=({},{}) on a locked board'.format(
                tile_id, direction))
        self.notify_observers()
    ...
    def rotate_ports(self):
        ...
        """
        for port in self.ports:
            port.tile_id = ((port.tile_id + 1) %
                            len(hexgrid.coastal_tile_ids())) + 1
            port.direction = hexgrid.rotate_direction(
                hexgrid.EDGE, port.direction, ccw=True)
        self.notify_observers()

    def set_terrain(self, terrain):
        self.tiles = [Tile(tile.tile_id, t, tile.number)
                      for t, tile in zip(terrain, self.tiles)]

    def set_numbers(self, numbers):
        self.tiles = [Tile(tile.tile_id, tile.terrain, n)
                      for n, tile in zip(numbers, self.tiles)]
